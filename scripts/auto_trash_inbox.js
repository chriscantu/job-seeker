...
// detect partial failures (moved < matched). Returns { isPartial, failures }.
function detectPartialFailure(stdout) {
  if (!stdout.startsWith('trashed:')) {
    return { isPartial: false, failures: [] };
  }
  const body = stdout.slice('trashed:'.length).trim();
  // Each pattern entry is `pattern=moved/matched` optionally followed by
  // ` (errors: ...)`. Split on whitespace but preserve error suffixes.
  const failures = [];
  // A simple approach: match `word=N/M` pairs.
  const entryRe = /(\S+?)=(\d+)\/(\d+)/g;
  let m;
  ...
  return { isPartial: failures.length > 0, failures };
}
...
function main() {
  ...
  // The AppleScript returns 0 even for its own error strings; inspect
  // stdout for the sentinel error markers.
  ...
  const { isPartial } = detectPartialFailure(result.stdout);
  if (isPartial) {
    return EXIT_PARTIAL;
  }
  return EXIT_OK;
