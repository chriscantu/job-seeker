...
### Automated validation — `scripts/validate-config.ts`

A script (run via `bun scripts/validate-config.ts`) that:
...
Skills instruct Claude to run `bun scripts/validate-config.ts` as the first step in "Before You Start" — if it exits non-zero, stop and surface the error message to the user.
...
1. Run `bun scripts/validate-config.ts` with both config files present — confirm `✓ Config valid`
...
9. Run `bun scripts/validate-config.ts` with a missing required field — confirm specific failure message
