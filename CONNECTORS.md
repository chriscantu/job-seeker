...
local CLI (`scripts/gmail.ts`) backed by the `googleapis` package and a single
OAuth2 flow.
...
| Gmail | Active | `scripts/gmail.ts` — Desktop-app OAuth2 flow via `googleapis`. Commands: `auth`, `profile`, `search`, `create-draft`, `trash`. Used by `follow-up` to draft follow-up messages. Credentials in `credentials/` (gitignored). Testing-mode tokens expire every 7 days. |
